...
class PageFragments(BaseModel):
    ...
    __table_args__ = (
        ...
        Index('idx_page_fragments_fragment_key', 'fragment_key'),
    ...
    @classmethod
    def get_all_active_for_page(cls, session, page_uuid):
    ...
    @classmethod
    def get_page_fragment_structure(cls, session, page_uuid):
        """Get a summary of all fragment_keys and their active versions for a page"""
        from sqlalchemy import func

        # Get all unique fragment_keys with their active version info
        active_fragments = session.query(
            cls.fragment_key,
            cls.version_number,
            cls.version_label,
            cls.content_type,
