...
def _check_permission(permission_name: str) -> bool:
    ...
    try:
        # Use your existing model registry
        from app.register.database import get_model
        
        role_permission_model = get_model('RolePermission')
        if not role_permission_model:
            return True  # Fail open if RBAC not loaded
        
        return role_permission_model.user_has_permission(
            g.session, user.uuid, permission_name
        )
    except Exception:
    ...
    try:
        ...
        for perm_name in _DISCOVERED_PERMISSIONS:
            if ':' in perm_name and perm_name != 'admin':
                ...
                existing = permission_model.find_by_name(session, perm_name)
                if not existing:
