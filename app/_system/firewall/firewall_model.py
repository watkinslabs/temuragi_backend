...
class Firewall(BaseModel):
    ...
    def get_all_patterns(cls, db_session):
        ...
        patterns = db_session.query(cls).filter(cls.active == True).order_by(cls.order, cls.ip_pattern).all()
        return patterns

    @classmethod
    def get_active_patterns(cls, db_session, pattern_type=None):
        """Get active IP patterns with optional type filter"""
        query = db_session.query(cls).filter(cls.active == True)
    ...
    def find_by_id(cls, db_session, pattern_id):
        ...
        return db_session.query(cls).filter(cls.uuid == pattern_id).first()
    ...
    def check_ip_access(cls, db_session, ip_address):
        ...
        patterns = db_session.query(cls).filter(cls.active == True).order_by(cls.order).all()
        ...
        allow_exists = db_session.query(cls).filter(cls.active == True, cls.ip_type == 'allow').first() is not None
        if allow_exists:
